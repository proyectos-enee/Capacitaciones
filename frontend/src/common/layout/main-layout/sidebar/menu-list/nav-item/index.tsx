...
const NavItem = ({ item, level }: any) => {
  ...
  const downBreakpoint = theme.breakpoints.down
    ? theme.breakpoints.down('lg')
    : 'lg';
  const matchesSM = useMediaQuery(downBreakpoint);
  ...
  const itemHandler = (id: any) => {
    customization.menuOpen(id);
    //  dispatch({ type: MENU_OPEN, id });
    if (matchesSM) customization.setMenu(false); //dispatch({ type: SET_MENU, opened: false });
  };
  ...
  useEffect(() => {
    ...
    if (currentIndex > -1) {
      ...
      customization.menuOpen(item.id);
    }
